...
class STID(nn.Module):
    def __init__(self,adj=None, **model_args):
        ...
        self.node_emb = nn.Embedding(100, self.node_dim)
        ...
        # )
        if self.if_time_in_day:
            # self.tranfsmer_day=nn.TransformerEncoderLayer(d_model=self.t_i_d_len, nhead=4)
            # encoder_layers = nn.TransformerEncoderLayer(d_model=self.t_i_d_len, nhead=4)
            # self.time_in_day_Encoder= nn.TransformerEncoder(encoder_layers, num_layers=3)
            self.time_in_day_emb= nn.Conv2d(in_channels=self.t_i_d_len, out_channels=self.temp_dim_tid, kernel_size=(1, 1), bias=True)
        ...
        # self.batch_norm=nn.BatchNorm2d(self.input_len,momentum=1)
...
# 整个模型存在两个问题，一个是seriesemb太大，一个是nodeemb太小
    def forward(self, history_data: torch.Tensor,history_data_2=None) -> torch.Tensor:
        ...
        embeddings=[]
        # history_data=self.batch_norm(history_data)
        if self.if_time_series:
            time_series_emb = self.time_series_emb_layer(history_data[..., 0:1])
            # time_in_day_data = history_data[..., 1:2]
            # embeddings.append(time_series_emb*torch.sigmoid(out)*2)
            embeddings.append(time_series_emb)
            if self.if_time_in_day:
                # his1=history_data[..., 0].view(history_data.shape[0],history_data.shape[1],history_data.shape[2],1)
                # his2=history_data_2[..., 0].view(history_data_2.shape[0],history_data_2.shape[1],history_data_2.shape[2],1)
                # out=self.tranfsmer_day(his2,his1)
        ...
        if self.if_node:
            ...
            out=self.node_emb(history_data.squeeze(1).long())
